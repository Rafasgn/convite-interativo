...
O arquivo fica em wwwroot/themes/<slug>/assets/teaser.png, servido como arquivo estático, e é usado como og:image da página pública /Convite/{token}. Não há geração nem manipulação de imagem em runtime: quando o admin clica em "gerar link", o sistema só cria o token e mostra a URL (ADR 0005); a imagem não entra nesse fluxo.
...
A página /Convite/{token} precisa renderizar as meta tags Open Graph apontando para a URL pública e absoluta de wwwroot/themes/<slug>/assets/teaser.png do tema daquele evento/convite.
